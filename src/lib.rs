...
) -> impl Fn(Request, Next) -> Pin<Box<dyn Future<Output = Response>>> {
    let authenticator = Arc::new(Mutex::new(authenticator));
    move |request, next| Box::pin(require_auth_middleware(authenticator.clone(), request, next))
}
...
async fn require_auth_middleware(
    ...
    match authenticator.lock().await.authenticate_request(request.headers()).await {
        Ok(auth) => return next.run(request).await,
